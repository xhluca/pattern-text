...
from pattern_text.text.tree import Tree, Text, Sentence, Slice, Chunk, PNPChunk, Chink, Word, table
from pattern_text.text.tree import SLASH, WORD, POS, CHUNK, PNP, REL, ANCHOR, LEMMA, AND, OR

DEFAULT = "default"

from pattern_text.helpers import encode_string, decode_string
...
class Model(object):
    ...
    def __init__(self, path="", classifier=None, known=set(), unknown=set()):
        ...
        """
        try:
            from pattern_text.vector import Classifier
            from pattern_text.vector import Perceptron
        except ImportError:
...
def commandline(parse=Parser().parse):
    ...
    if o.version:
        sys.path.insert(0, os.path.join(MODULE, "..", ".."))
        from pattern_text import __version__
        print(__version__)
        sys.path.pop(0)
